...
export function openAICoachPanel(){
 ...
 setTimeout(()=>panel.querySelector<HTMLTextAreaElement>('#coach-input')?.focus(),0);
}

export function mountAICoach(){
 let btn=document.querySelector<HTMLButtonElement>('#ai-coach-btn');
 if(!btn){btn=document.createElement('button');btn.id='ai-coach-btn';btn.innerHTML='✦ DELIONARYO AI COACH<br><small>24/7 Transformation Guide</small>';btn.style.cssText='position:fixed;right:22px;bottom:22px;z-index:9999;background:#fbbf24;color:#1c1917;padding:14px;border-radius:18px;font-weight:900;box-shadow:0 12px 35px #0005';btn.onclick=openAICoachPanel;document.body.appendChild(btn);}
}
