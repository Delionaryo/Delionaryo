...
export default async function handler(req:any,res:any){
  ...
  const jsonMode=String(req.query?.mode||'')==='json';
  const refresh=parseCookies(req).dl_sso_refresh||'';
  if(!refresh){
    res.setHeader('Set-Cookie',clearCookies());
    if(jsonMode)return res.status(401).json({authenticated:false,refreshed:false});
    res.setHeader('Location',startUrl(target));return res.status(302).end();
  }
  try{
    ...
    if(jsonMode)return res.status(200).json({authenticated:true,refreshed:true});
    res.setHeader('Location',target);return res.status(302).end();
  }catch(_){
    if(jsonMode)return res.status(503).json({authenticated:false,refreshed:false,error:'AUTH_REFRESH_UNAVAILABLE'});
    res.setHeader('Location',startUrl(target));return res.status(302).end();
  }
}
