...
export default async function handler(req:any,res:any){
  ...
  if(!refresh){res.setHeader('Set-Cookie',clearCookies());res.setHeader('Location',startUrl(target));return res.status(302).end()}
  try{
    ...
    if(!response.ok||!payload?.access_token||!payload?.refresh_token){res.setHeader('Set-Cookie',clearCookies());res.setHeader('Location',startUrl(target));return res.status(302).end()}
    ...
    res.setHeader('Location',startUrl(target));
    return res.status(302).end();
  }
}
